var excelController = function (analyzer) {
    ...
    function dateParse(dateString) {
        ...
        if(splitStrings.length == 3) {
            ...
            [splitStrings[0], splitStrings[1]] = [splitStrings[1], splitStrings[0]];
            splitStrings.reverse();
            splitStrings = splitStrings.map((val, index) => {
                var intVal = parseInt(val);
                if(index !== 0) intVal -= 1;
                return intVal;
            });
            date = new Date(...splitStrings);
        } else {
    ...
    function initSales() {
        ...
        var sale = function(id, pid, qty, discount, pinCode, city, state, oDate) {
            this.id = id;
            this.city = city;
            this.state = state;
            this.pid = pid;
    ...
    function initProducts() {
        ...
        var product = function(id, name, sellPrice, category, subCategory) {
            this.id = id;
            this.name = name;
        ...
        productDB.JSON.forEach(function (cur) {
            var pd = new product(
                cur[heads.pid],
                cur[heads.name],
                parseFloat(cur[heads.salesPrice]),
                cur[heads.category],
                cur[heads.subCategory]
            );
    ...
    return {
        ...
        parseData: function(source, DBtype, xlsxflag, getHeads) {
            return new Promise(function(resolve, reject) {
                var workbook;
                if (!getHeads) {
        ...
        initHeads: function(heads, fileType) {
