...
var controller = function (UICtrl, excelCtrl) {
    ...
    function uploadFile(formData) {
        ...
        $.ajax({
            ...
            error: function error() {
                for (var _len = arguments.length, status = Array(_len), _key = 0; _key < _len; _key++) {
                    status[_key] = arguments[_key];
                }
    ...
    function setupEventListeners() {
        $(document).ready(function () {
            ...
            inputFile.change(function () {
                ...
                if (ext == 'xls' || ext == 'xlsx') {
                    excelCtrl.parseData(file, 'salesFile', xlsxCheck(file.name), true).then(function (tableHeads) {
    ...
    return {
        ...
        initFormData: function initFormData(sheets) {
