...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 15, 2019 at 08:06 AM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `userdetails`  AS  select `l`.`username` AS `USERNAME`,`l`.`joined_time` AS `JOINED_TIME`,((`u`.`fname` <> 0) or ' ' or (`u`.`lname` <> 0)) AS `FULLNAME`,`u`.`email` AS `email`,`u`.`contact_no` AS `contact_no` from (`login` `l` join `user_details` `u`) where (`l`.`lid` = `u`.`lid`) ;
